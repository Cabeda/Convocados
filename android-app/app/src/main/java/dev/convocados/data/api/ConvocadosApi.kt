...
class ConvocadosApi @Inject constructor(private val client: ApiClient) {
    ...
    suspend fun reportAppOpen(): Map<String, Boolean> =
        client.post("/api/me/app-open", mapOf("platform" to "android", "appVersion" to BuildConfig.VERSION_NAME))
    ...
    suspend fun addPlayer(
        ...
        val body = AddPlayerRequest(name, linkToAccount, email)
        return if (idempotencyKey != null) {
            client.postWithHeader("/api/events/$eventId/players", "Idempotency-Key", idempotencyKey, body)
        } else {
            client.post("/api/events/$eventId/players", body)
        }
    }
