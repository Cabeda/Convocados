...
- Merges performed before this behaviour are repaired automatically on deploy: the `backfill_merged_player_identity` migration enqueues a one-shot `backfill_merged_identity` scheduler job that collapses the split identities and rebuilds the affected ELO. The same repair can be run manually with `npm run db:backfill:merged-identity` (dry run by default; `--apply` to write).
