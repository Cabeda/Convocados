...
import { POST as adoptPOST } from "~/pages/api/events/[id]/adopt";
import { POST as addPlayerPOST } from "~/pages/api/events/[id]/players";
...
const mockDrain = vi.fn().mockResolvedValue(1);
vi.mock("~/lib/notificationQueue.server", () => ({
  enqueueNotification: (...args: unknown[]) => mockEnqueue(...args),
  drainNotificationQueue: (...args: unknown[]) => mockDrain(...args),
}));
...
describe("POST /api/events/[id]/adopt", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await adoptPOST(ctx("e1"));
    expect(res.status).toBe(401);
  });

  it("returns 404 when event not found", async () => {
    mockGetSession.mockResolvedValue({ user: { id: "u1", name: "U" } } as any);
    const res = await adoptPOST(ctx("nonexistent"));
  ...
  it("returns 409 when the event already has an owner", async () => {
    ...
    mockGetSession.mockResolvedValue({ user: { id: "u1", name: "U" } } as any);
    const res = await adoptPOST(ctx(pickup.id));
  ...
  it("returns 409 when the event is not a pickup (manual source)", async () => {
    ...
    mockGetSession.mockResolvedValue({ user: { id: "u1", name: "U" } } as any);
    const res = await adoptPOST(ctx(event.id));
    expect(res.status).toBe(409);
  });
  ...
  it("adopts the pickup: sets the owner, keeps it public, notifies followers", async () => {
    ...
    expect(updated!.adoptedAt).not.toBeNull();
  ...
  it("blocks joining an un-adopted pickup until it is adopted", async () => {
    const pickup = await pickupEvent();
    mockGetSession.mockResolvedValue({ user: { id: "u1", name: "Joiner" } } as any);
    const res = await addPlayerPOST({
      params: { id: pickup.id },
      request: new Request(`http://localhost/api/events/${pickup.id}/players`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name: "Joiner" }),
      }),
    } as any);
