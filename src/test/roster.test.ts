...
async function addGameParticipant(gameId: string, eventId: string, name: string, order: number, archivedAt: Date | null = null) {
  const ep = await prisma.eventPlayer.create({ data: { eventId, name } });
  await prisma.gameParticipant.create({ data: { gameId, eventPlayerId: ep.id, order, archivedAt } });
}
